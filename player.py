...
class Player:
    ...
    def go_to_jail(self):
        ...
        observer.Event("update_state", f"{self.name} has been sent to jail! roll doubles to escape or pay the consequences!")

    def leave_jail(self):
        self.__in_jail = False
        self.__jail_turns = 0
        observer.Event("update state", f"{self.name}  is out of jail")
    ...
    def move(self, spaces):
        ...
        if self.__in_jail:
            observer.Event("update state", f"{self.__name} is in jail and cannot move.")
            return
